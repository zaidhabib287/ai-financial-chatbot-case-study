...
async def test_transfer_blocked_by_sanctions(client):
    ...
    ad = await client.post(
        "/api/v1/auth/login", data={"username": "admin4", "password": "AdminPass1"}
    )
    ...
    cd = await client.post(
        "/api/v1/auth/login", data={"username": "cust2", "password": "CustPass1"}
    )
    ...
    await client.post(
        "/api/v1/admin/documents/upload", headers=hadmin, files=files, data=data
    )
    ...
    await client.post(
        f"/api/v1/admin/users/{cust_id}/credit",
        headers=hadmin,
    ...
    b = await client.post(
        ...
        json={
            ...
            "country": "Iran",  # blacklisted by RAG
        },
    )
    ben = b.json()
