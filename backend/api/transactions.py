...
async def validate_transfer(
    ...
    try:
        ...
        async def _rag_limits_and_sanctions():
            ...
            for q in queries:
                q_emb = rag_manager.embedding_generator.generate_single_embedding(q)  # :contentReference[oaicite:3]{index=3}
                hits = rag_manager.vector_store.search(q_emb, k=5)                    # :contentReference[oaicite:4]{index=4}
                ...
                extracted_rules.extend(rag_manager.rule_extractor.extract_rules(context_text))
                s_lists = rag_manager.rule_extractor.extract_sanctions_list(context_text)
                sanctions["countries"].update(map(str.strip, s_lists.get("countries", [])))
                sanctions["entities"].update(map(str.strip, s_lists.get("entities", [])))
            ...
            for r in extracted_rules:
                if r["rule_type"] == "transaction_limit" and isinstance(r["rule_value"], (int, float)):
                    per_tx_limit = float(r["rule_value"]) if per_tx_limit is None else per_tx_limit
                if r["rule_type"] == "transfer_limit" and isinstance(r["rule_value"], (int, float)):
                    daily_limit = float(r["rule_value"]) if daily_limit is None else daily_limit

            return per_tx_limit, daily_limit, sanctions

        rag_per_tx_limit, rag_daily_limit, rag_sanctions = await _rag_limits_and_sanctions()
    except Exception:
        # Be conservative but resilient if vector DB is empty or RAG errors out
        rag_per_tx_limit, rag_daily_limit, rag_sanctions = None, None, {"countries": set(), "entities": set()}

    # Effective limits: RAG overrides config if available:contentReference[oaicite:6]{index=6}:contentReference[oaicite:7]{index=7}
    effective_per_tx = float(rag_per_tx_limit) if rag_per_tx_limit else float(settings.per_transaction_limit)
    effective_daily  = float(rag_daily_limit)  if rag_daily_limit  else float(user.daily_limit)

    # 1) Per-transaction limit (RAG-aware)
    if amount > effective_per_tx:
        return False, f"Amount exceeds per-transaction limit of {effective_per_tx:g} BHD"
    ...
    if rag_sanctions and rag_sanctions.get("countries"):
        ...
        if any(ben_country_norm == c.strip().lower() for c in rag_sanctions["countries"]):
            return False, "Transfer blocked: Beneficiary country appears on a sanctioned/blacklisted list (RAG)."

    # 3b) Mock API (second line of defense):contentReference[oaicite:8]{index=8}
    sanctions_check = await mock_banking_api.check_sanctions(beneficiary.name, beneficiary.country)
