...
class RAGManager:
    ...
    async def process_document(self, file_path: str, document_id: str, document_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            sanctions = self.rule_extractor.extract_sanctions_list(doc_data['cleaned_text'])
            
            # Generate embeddings for chunks
            chunks_with_embeddings = self.embedding_generator.process_chunks(doc_data['chunks'])
    ...
    async def query_documents(self, query: str, k: int = 5, document_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        results = self.vector_store.search(query_embedding, k=k, filter_metadata=filter_metadata)
    ...
    async def check_sanctions(self, name: str, country: str) -> Dict[str, Any]:
        ...
        name_results = await self.query_documents(name_query, k=3, document_type='sanctions_list')
        ...
        country_results = await self.query_documents(country_query, k=3, document_type='sanctions_list')
        ...
        for result in name_results:
            if result['similarity'] > 0.7 and name.lower() in result['text'].lower():
                is_sanctioned = True
                reasons.append(f"Name found in sanctions document: {result['metadata']['file_name']}")
        
        for result in country_results:
            if result['similarity'] > 0.7 and country.lower() in result['text'].lower():
                is_sanctioned = True
                reasons.append(f"Country found in sanctions document: {result['metadata']['file_name']}")
        
        return {
            'is_sanctioned': is_sanctioned,
            'reasons': reasons,
            'confidence': max([r['similarity'] for r in name_results + country_results], default=0.0)
        }
    ...
    async def get_compliance_rules(self, rule_type: str) -> List[Dict[str, Any]]:
        ...
        results = await self.query_documents(query, k=5, document_type='compliance_rules')
